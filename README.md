...
3. Всегда должно быть указано базовое семейство в конце списка шрифтов, например, `sans-serif`.
...
2. Скрипт для каждой страницы должен лежать в отдельном файле.
3. Общие скрипты для нескольких страниц должны лежать каждый в своем файле.
4. Скрипты должны быть по возможности подключены с CDN
5. Скрипты должны начинаться с `use strict`;
...
7. В скриптах не допускается создавать уютный филиал callback hell'a
    ...
    ```
8. Используйте `var` для создания функций.
